...
def list_cbz_files(folder_path):
    """Lists all .cbz files in the specified folder."""
    cbz_files = []
    for file_name in os.listdir(folder_path):
        if file_name.endswith('.cbz'):
            cbz_files.append(file_name)
            print(f"Extract cbz found as: {file_name}")
...
def cbz_to_pdf(cbz_file, output_pdf):
    ...
    input_cbz_files = list_cbz_files(input_folder)

    for cbz_file in input_cbz_files:

        print(f"looking for: {cbz_file}")
...
# Example usage
input_folder = 'D:\\d3\\daniel way\\cbz to epub\\input'       
cbz_file_path = 'D:\\d3\\daniel way\\cbz to epub\\input.cbz'
